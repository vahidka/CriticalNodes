...
class DNode(Node):
	...
	def run(self):
		...
		#self.Bu[self.id]=len(self.neighbor.keys());
		
		for i in self.neighbor.keys():
				receivedbytes=receivedbytes+(len(g.nodes[i].neighbor.keys())*2)+2;
		sentbytes=sentbytes+2+(2*len(self.neighbor.keys()))
		score=0;
		for i in self.neighbor.keys():
			score=score+len(g.nodes[i].neighbor.keys());

		score1=len(self.neighbor.keys())/score;
		#score1=score1*len(self.neighbor.keys());
		self.Bu[self.id]=score1;
...
for filename in os.listdir(os.getcwd()+"/topo/"):
	for k in range(5,21,5):
		with open(os.path.join(os.getcwd()+"/topo/", filename), 'r') as f: # open in readonly mode
			line=f.readline();
